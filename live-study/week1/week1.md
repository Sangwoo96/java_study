...
C의 경우는, 우리가 알 수 있는 고급언어인 소스코드(```*.c```)가 컴파일되어 어셈블리어(```*.obj```)로 변환될 때 바이너리 파일로 변환된다. 

(0과 1) 어셈블리어(```*.obj```)는 컴퓨터가 이해할 수 있지만 완전한 기계어가 아니기 때문에 실행될 수는 없다. 

변환된 어셈블리어(```*.obj```)는 링커에 의해 실행 가능한 실행파일(```*.exe```)로 변환될 때 여러 개의 코드와 데이터를 모아서 연결하여 메모리에서 실행 가능 한 파일로 만든다. 

이러한 과정을 거쳐야 비로소 컴퓨터가 바로 실행 할 수 있는 기계어가 된다.
...
* **기계어는 특정한 언어가 아니다.** CPU 제조사마다 CPU를 만들때 사용할 수 있는 명령어를 공개하며, 이를 '기계어'라고 부를 뿐이다. 만약 CPU가 변경되면 기계어도 달라진다. 같은 동작을 하는 명령어이지만 0과 1을 인식하는 단위나 방법이 다를 수 있기 때문이다.
...
전통적으로 프로그램을 만드는 방식은 다음과 같이 두 가지가 있다.
...
* JIT 컴파일러의 동작방식
    - 바이트코드를 기계어로 컴파일해주는 역활
    - 인터프리터 효율을 높이기 위해, 인터프리터가 반복되는 코드를 발견하면 JIT 컴파일러로 반복되는 코드를 모두 기계어로 바꿔둔다. 그 다음부터 인터프리터는 기계어로 컴파일된 코드를 바로 사용한다.
* JIT 컴파일러의 장점
    - 기계어 변환(기계어로 바꾸는 작업)은 코드가 실행되는 과정에 실시간으로 일어나며(그래서 Just-In-Time이다), 전체 코드의 필요한 부분만 변환한다. 기계어로 변환된 코드는 캐시에 저장되기 때문에 재사용시 컴파일을 다시 할 필요가 없고 컴파일된 코드를 바로 사용한다.
    - 일반적인 인터프러터 언어에 비해 훨씬 좋은 성능을 낸다. 또한, 경우에 따라 정적 컴파일러 언어보다 좋은 성능을 내곤 하는데, 이는 실행 과정에 컴파일을 할 수 있기 때문이다.

JIT은 `javac`(컴파일)와 관련이 없다. 컴파일된 `.class` (바이트코드)를 기계어로 번역할때 사용된다.

JIT과 인터프리터는 동시에 진행된다. 따라서 자바가 c보다 느리다는 것은 옛날 이야기이다. 지금은 비슷하다고 많이 이야기한다.
...
자바 언어로 개발하기 위해서는 JDK를 설치해야하고 프로그램을 실행하기 위해서는 JRE를 설치해야한다.

사실 JDK에 JRE도 포함되어 설치된다. 자바 9 이상부터는 JRE가 사라졌다.
